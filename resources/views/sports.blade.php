...
<main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <!-- Must Watch Section -->
        <section>
            <h2 class="text-xl font-bold mb-4">SUIVEZ NOS PROGRAMMES SPORT</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div class="md:col-span-2">
                    ...
                    @elseif (!empty($video->cover_image))
                        <!-- Cover Image -->
                        <img
                            src="{{ asset('storage/covers/' . $video->cover_image) }}"
                ...
                <div class="space-y-4">
                    ...
                    <a href="#" class="flex items-start gap-4">
                        {{-- <img src="https://via.placeholder.com/120x70" class="w-28 h-16 object-cover" alt="Thumbnail"> --}}
                        <video src="{{ asset('images/news.mp4') }}" class="w-28 h-16 object-cover"></video>

                        <div>
                            <p class="text-blue-600 font-bold text-sm leading-snug">
                                {{ ['MAURIS FERMENTUM TORTOR NON ENIM ALIQUET CONDIMENTUM', 'TORTOR NON ENIM ALIQUET CONDIMENTUM NAM ALIQUAM', 'NAM ALIQUAM PRETIUM FEUGIAT DUIS SEM EST VIVERRA EU INTERDUM'][$i - 1] }}
                            </p>
                            <p class="text-gray-500 text-xs mt-1 flex gap-x-2">
                                <i class="fa fa-user mr-1"></i> Admin
                                <i class="fa fa-clock mr-1"></i> {{ now()->subDays(1)->diffForHumans() }}
                            </p>
                        </div>
        ...
        <section class="mt-8">
            ...
            <form action="{{ url('') }}" method="POST" class="mb-6">
                @csrf
